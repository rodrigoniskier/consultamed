...
export function PatientSecretaryDashboard() {
  ...
  const [newAppt, setNewAppt] = useState({ ...EMPTY_APPOINTMENT });
  ...
  const fetchShifts = async () => {
    ...
    if (data) {
      const validShifts = data.filter(
        (shift: any) => shift.rooms && String(shift.rooms.location_id) === String(filters.location_id),
      );
      setShifts(validShifts);
    }
  };

  const openAppointmentModal = (shift: DoctorShift) => {
    setSelectedShift(shift);
    setNewAppt({ ...EMPTY_APPOINTMENT });
    setShowModal(true);
  };
  ...
  const handleCreateAppointment = async (e: React.FormEvent) => {
    ...
    if (newAppt.has_physical_record !== 'sim' && newAppt.has_physical_record !== 'nao') {
      alert('Informe se o paciente possui ficha/prontuário físico para este atendimento.');
      return;
    }
    ...
    if (newAppt.new_patient) {
      ...
      finalPatientId = String(newPat.id);
    ...
    const notes = [
      `Ficha/prontuário físico: ${newAppt.has_physical_record === 'sim' ? 'SIM' : 'NÃO'}`,
    ];

    if (newAppt.has_referral) {
      notes.push(`Encaminhado por: ${newAppt.referred_by || 'Não informado'}`);
    }
      ...
      <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-6 shrink-0 z-10 shadow-sm">
        ...
        <div className="flex items-center gap-4">
          <div className="text-sm font-medium text-slate-600">
            {profile?.full_name}{' '}
            <span className="text-xs ml-1 bg-blue-100 text-blue-700 px-2 py-0.5 rounded font-bold uppercase">
              Agendamento
            </span>
          </div>
          <button
          ...
          <div className="flex flex-col md:flex-row gap-4">
            <div className="flex-1">
              ...
              <select
                ...
                className="w-full rounded-lg border-slate-300 p-2.5 border text-sm"
              >
          ...
          <div className="flex-1 bg-white rounded-xl border border-slate-200 overflow-hidden flex flex-col">
            ...
            <div className="grid grid-cols-7 gap-px bg-slate-200 flex-1">
              {['Dom', 'Seg', 'Ter', 'Qua', 'Qui', 'Sex', 'Sáb'].map((dayName) => (
                <div key={dayName} className="bg-slate-100 p-2 text-center text-xs font-bold text-slate-500 uppercase">
                  {dayName}
                </div>
              ))}
              ...
              {days.map((day) => {
                ...
                return (
                  <div
                    key={dateStr}
                    className={`bg-white min-h-[100px] p-2 flex flex-col ${!hasShift || isPast ? 'opacity-50' : 'hover:bg-blue-50'}`}
                  >
                    <span className={`text-sm font-semibold mb-2 ${hasShift ? 'text-blue-900' : 'text-slate-400'}`}>
                      {format(day, 'd')}
                    </span>
                    ...
                    {hasShift && !isPast && (
                      <div className="flex flex-col gap-1 mt-auto">
                        {dayShifts.map((shift) => (
                          <button
                            key={shift.id}
                            onClick={() => openAppointmentModal(shift)}
                            className="text-left text-[10px] bg-blue-100 text-blue-800 px-2 py-1 rounded truncate hover:bg-blue-200"
                          >
                            {shift.turn}
                          </button>
                        ))}
          ...
          <div className="bg-white w-full max-w-lg rounded-2xl p-8 shadow-2xl max-h-screen overflow-y-auto">
            ...
            <form onSubmit={handleCreateAppointment} className="space-y-4">
              <div className="flex bg-slate-100 p-1 rounded-lg">
                <button
                  type="button"
                  onClick={() => setNewAppt({ ...newAppt, new_patient: false })}
                  className={`flex-1 py-1.5 text-sm font-medium rounded-md ${!newAppt.new_patient ? 'bg-white shadow-sm text-blue-700' : 'text-slate-500'}`}
                >
                  Paciente Existente
                </button>
                <button
                  type="button"
                  onClick={() => setNewAppt({ ...newAppt, new_patient: true, patient_id: '' })}
                  className={`flex-1 py-1.5 text-sm font-medium rounded-md ${newAppt.new_patient ? 'bg-white shadow-sm text-blue-700' : 'text-slate-500'}`}
                >
                  Novo Paciente
                </button>
                  ...
                  >
                    ...
                    {patients.map((patient) => (
                      <option key={patient.id} value={patient.id}>
                        {patient.full_name} {patient.cpf ? `(CPF: ${patient.cpf})` : ''}
                      </option>
                    ))}
                  </select>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      value={newAppt.rg}
                      onChange={(e) => setNewAppt({ ...newAppt, rg: e.target.value })}
                      className="w-full rounded-lg border-slate-300 p-2.5 border text-sm"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      type="text"
                      value={newAppt.cpf}
                      onChange={(e) => setNewAppt({ ...newAppt, cpf: e.target.value })}
                      className="w-full rounded-lg border-slate-300 p-2.5 border text-sm"
                    />
                  </div>
                  <div className="col-span-2">
                    ...
                    <input
                      type="text"
                      value={newAppt.mother_name}
                      onChange={(e) => setNewAppt({ ...newAppt, mother_name: e.target.value })}
                      className="w-full rounded-lg border-slate-300 p-2.5 border text-sm"
                    />
                  </div>
                </div>
              ...
              <div className="rounded-xl border border-amber-200 bg-amber-50 p-4">
                <label className="block text-sm font-semibold text-amber-950 mb-1">
                  O paciente possui ficha/prontuário físico para este atendimento?
                </label>
                <select
                  required
                  value={newAppt.has_physical_record}
                  onChange={(e) => setNewAppt({ ...newAppt, has_physical_record: e.target.value })}
                  className="w-full rounded-lg border-amber-300 p-2.5 border text-sm bg-white"
                >
                  <option value="">Selecione...</option>
                  <option value="sim">Sim — já possui ficha/prontuário físico</option>
                  <option value="nao">Não — será necessário criar uma ficha</option>
                </select>
                <p className="text-xs text-amber-800 mt-2">
                  Esta informação deve ser confirmada em cada novo agendamento, inclusive para pacientes já cadastrados.
                </p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 mt-4">
                ...
                <div className="col-span-2 mt-2">
                  <label className="flex items-center gap-2 cursor-pointer text-sm font-medium text-slate-700">
                    <input
                      type="checkbox"
                      checked={newAppt.has_referral}
                      onChange={(e) => setNewAppt({ ...newAppt, has_referral: e.target.checked })}
                ...
                {newAppt.has_referral && (
                  <div className="col-span-2">
                    ...
                    <input
                      type="text"
                      required
                      value={newAppt.referred_by}
                      onChange={(e) => setNewAppt({ ...newAppt, referred_by: e.target.value })}
                      className="w-full rounded-lg border-slate-300 p-2.5 border text-sm"
                    />
                  </div>
                )}
              ...
              <div className="flex justify-end gap-3 pt-6 mt-4 border-t border-slate-100">
                <button
                  ...
                  className="px-4 py-2 border border-slate-300 rounded-lg text-slate-700 bg-white hover:bg-slate-50 font-medium"
                >
                  Cancelar
                </button>
                <button
